...
window.setInterval( ->
	...
	# only do this if in proper current scene
	for pauseSegment in endScenePauseSegments
		if currTime > pauseSegment[0] and currTime < pauseSegment[1] and currTime > sceneStarts[currScene]
			videoLayer.player.pause()
			playButton.image = "images/play.png"
	...
	if currScene not in endScenes and currTime > sceneStarts[currScene + 1] - 2.0
		pauseHelper()
, 100)
